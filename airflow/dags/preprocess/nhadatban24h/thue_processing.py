...
class Nhadatban24hThueProcessor:
    ...
    def preprocess(self):
        cleaned_nhadatban24h_thue = self.clean_dataframe(self.data)
        ...
        cleaned_nhadatban24h_thue['Price'] = cleaned_nhadatban24h_thue.apply(self.convert_to_vnd,axis=1)
        cleaned_nhadatban24h_thue['Location'] = cleaned_nhadatban24h_thue['Location'].apply(self.clean_location)
        cleaned_nhadatban24h_thue['Standardized_Location'] = cleaned_nhadatban24h_thue['Location'].apply(lambda address: self.standardize_address(address))
        cleaned_nhadatban24h_thue['Standardized_Location'] = cleaned_nhadatban24h_thue.apply(self.fill_empty_location, axis=1)
        # cleaned_nhadatban24h_thue.to_csv('./processed_data/nhadatban24h/processed_nhadatban24h_thue.csv', index=False)
        cleaned_nhadatban24h_thue['Best_Match_Location'] = cleaned_nhadatban24h_thue.apply(self.find_best_match, axis=1)

        cleaned_nhadatban24h_thue[['Ward', 'District', 'City']] = cleaned_nhadatban24h_thue['Best_Match_Location'].apply(lambda x: pd.Series(self.split_location(x)))
        num_none = cleaned_nhadatban24h_thue[['Ward', 'District', 'City']].isna().sum()

        print("Number of None values in each column:")
        print(num_none)
        cleaned_nhadatban24h_thue.to_csv(self.output_path, index=False)
        return cleaned_nhadatban24h_thue
