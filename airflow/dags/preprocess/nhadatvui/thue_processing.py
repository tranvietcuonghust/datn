...
class NhadatvuiThueProcessor():
    def __init__(self, input_path, output_path):
        self.name = "nhadatvui_thue_daily"
    ...
    def preprocess(self):
        cleaned_nhadatvui_thue = self.clean_dataframe(self.data)
        cleaned_nhadatvui_thue['Acreage'] = cleaned_nhadatvui_thue['Acreage'].str.replace(' m²', '')
        cleaned_nhadatvui_thue['Created_date'] = cleaned_nhadatvui_thue['Created_date'].str.replace('/', '-')
        cleaned_nhadatvui_thue['Created_date'] = pd.to_datetime(cleaned_nhadatvui_thue['Created_date'], format='%d-%m-%Y')
        cleaned_nhadatvui_thue['Created_date'] = cleaned_nhadatvui_thue['Created_date'].dt.strftime('%Y-%m-%d')
        cleaned_nhadatvui_thue= cleaned_nhadatvui_thue.sort_values(by='Created_date', ascending=False)
        cleaned_nhadatvui_thue['Price'] = cleaned_nhadatvui_thue.apply(self.convert_to_vnd, axis =1)
        cleaned_nhadatvui_thue['Location'] = cleaned_nhadatvui_thue['Location'].apply(self.clean_location)
        # cleaned_nhadatvui_thue['Location'].to_csv('./processed_data/nhadatvui/processed_nhadatvui_thue.csv', index=False)
        cleaned_nhadatvui_thue['Standardized_Location'] = cleaned_nhadatvui_thue['Location'].apply(lambda address: self.standardize_address(address))
        cleaned_nhadatvui_thue['Standardized_Location'] = cleaned_nhadatvui_thue.apply(self.fill_empty_location, axis=1)
        # cleaned_nhadatvui_thue.to_csv('./processed_data/nhadatvui/processed_nhadatvui_thue.csv', index=False)
        cleaned_nhadatvui_thue['Best_Match_Location'] = cleaned_nhadatvui_thue.apply(self.find_best_match, axis=1)

        cleaned_nhadatvui_thue[['Ward', 'District', 'City']] = cleaned_nhadatvui_thue['Best_Match_Location'].apply(lambda x: pd.Series(self.split_location(x)))
        num_none = cleaned_nhadatvui_thue[['Ward', 'District', 'City']].isna().sum()

        print("Number of None values in each column:")
        print(num_none)
        cleaned_nhadatvui_thue.to_csv(self.output_path, index=False)
        return cleaned_nhadatvui_thue   
