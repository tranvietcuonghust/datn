...
class NhadatvuiBanProcessor():
    ...
    def preprocess(self):
        cleaned_nhadatvui_ban = self.clean_dataframe(self.data)
        ...
        cleaned_nhadatvui_ban['Price'] = cleaned_nhadatvui_ban.apply(self.convert_to_vnd, axis=1)
        cleaned_nhadatvui_ban['Location'] = cleaned_nhadatvui_ban['Location'].apply(self.clean_location)
        # cleaned_nhadatvui_ban['Location'].to_csv('./processed_data/nhadatvui/processed_nhadatvui_ban.csv', index=False)
        cleaned_nhadatvui_ban['Standardized_Location'] = cleaned_nhadatvui_ban['Location'].apply(lambda address: self.standardize_address(address))
        cleaned_nhadatvui_ban['Standardized_Location'] = cleaned_nhadatvui_ban.apply(self.fill_empty_location, axis=1)
        # cleaned_nhadatvui_ban.to_csv('./processed_data/nhadatvui/processed_nhadatvui_ban.csv', index=False)
        cleaned_nhadatvui_ban['Best_Match_Location'] = cleaned_nhadatvui_ban.apply(self.find_best_match, axis=1)

        cleaned_nhadatvui_ban[['Ward', 'District', 'City']] = cleaned_nhadatvui_ban['Best_Match_Location'].apply(lambda x: pd.Series(self.split_location(x)))
        num_none = cleaned_nhadatvui_ban[['Ward', 'District', 'City']].isna().sum()

        print("Number of None values in each column:")
        print(num_none)
        cleaned_nhadatvui_ban.to_csv(self.output_path, index=False)
        return cleaned_nhadatvui_ban   
