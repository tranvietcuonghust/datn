...
class Nhadat247BanProcessor:
    def __init__(self, input_path, output_path):
        self.name = "nhadat247_ban_daily"
    ...
    def preprocess(self):
        cleaned_nhadat247_ban = self.clean_dataframe(self.data)
        ...
        cleaned_nhadat247_ban['Created_date'] = cleaned_nhadat247_ban.apply(self.replace_dates, axis=1)
        ...
        cleaned_nhadat247_ban['Price'] = cleaned_nhadat247_ban.apply(self.convert_to_vnd,axis=1)
        cleaned_nhadat247_ban['Location'] = cleaned_nhadat247_ban['Location'].apply(self.clean_location)
        # cleaned_nhadat247_ban['Location'].to_csv('./processed_data/nhadat247/processed_nhadat247_ban.csv', index=False)
        cleaned_nhadat247_ban['Standardized_Location'] = cleaned_nhadat247_ban['Location'].apply(lambda address: self.standardize_address(address))
        cleaned_nhadat247_ban['Standardized_Location'] = cleaned_nhadat247_ban.apply(self.fill_empty_location, axis=1)
        # cleaned_nhadat247_ban.to_csv('./processed_data/nhadat247/processed_nhadat247_ban.csv', index=False)
        cleaned_nhadat247_ban['Best_Match_Location'] = cleaned_nhadat247_ban.apply(self.find_best_match, axis=1)

        cleaned_nhadat247_ban[['Ward', 'District', 'City']] = cleaned_nhadat247_ban['Best_Match_Location'].apply(lambda x: pd.Series(self.split_location(x)))
        num_none = cleaned_nhadat247_ban[['Ward', 'District', 'City']].isna().sum()

        print("Number of None values in each column:")
        print(num_none)
        cleaned_nhadat247_ban.to_csv(self.output_path, index=False)
        return cleaned_nhadat247_ban
