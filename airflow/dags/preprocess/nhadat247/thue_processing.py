...
class Nhadat247ThueProcessor:
    ...
    def preprocess(self):
        cleaned_nhadat247_thue = self.clean_dataframe(self.data)
        cleaned_nhadat247_thue = cleaned_nhadat247_thue.drop(['Facade'], axis=1)
        cleaned_nhadat247_thue['Acreage'] = cleaned_nhadat247_thue['Acreage'].str.replace('[^\d]', '', regex=True)
        cleaned_nhadat247_thue = cleaned_nhadat247_thue.astype(str)
        cleaned_nhadat247_thue['Num_floors'] = cleaned_nhadat247_thue['Num_floors'].str.replace('[^\d]', '', regex=True)
        cleaned_nhadat247_thue['Num_rooms'] = cleaned_nhadat247_thue['Num_rooms'].str.replace('[^\d]', '', regex=True)
        cleaned_nhadat247_thue['Num_toilets'] = cleaned_nhadat247_thue['Num_toilets'].str.replace('[^\d]', '', regex=True)
        cleaned_nhadat247_thue['Street_size'] = cleaned_nhadat247_thue['Street_size'].str.replace('[^\d]', '', regex=True)

        cleaned_nhadat247_thue['Crawled_date_2'] = pd.to_datetime(cleaned_nhadat247_thue['Crawled_date'])
        cleaned_nhadat247_thue['Crawled_date_2'] = cleaned_nhadat247_thue['Crawled_date_2'].dt.strftime('%d/%m/%Y')

        cleaned_nhadat247_thue['Created_date'] = cleaned_nhadat247_thue.apply(self.replace_dates, axis=1)
        cleaned_nhadat247_thue['Created_date'] = pd.to_datetime(cleaned_nhadat247_thue['Created_date'], format='%d/%m/%Y').dt.strftime('%Y-%m-%d')
        cleaned_nhadat247_thue.drop('Crawled_date_2',axis=1, inplace=True)
        cleaned_nhadat247_thue =  cleaned_nhadat247_thue.sort_values(by='Created_date', ascending=False)
        # print(cleaned_nhadat247_thue['Price'].unique())
        cleaned_nhadat247_thue['Price'] = cleaned_nhadat247_thue.apply(self.convert_to_vnd,axis=1)
        cleaned_nhadat247_thue['Location'] = cleaned_nhadat247_thue['Location'].apply(self.clean_location)
        # cleaned_nhadat247_thue['Location'].to_csv('./processed_data/nhadat247/processed_nhadat247_thue.csv', index=False)
        cleaned_nhadat247_thue['Standardized_Location'] = cleaned_nhadat247_thue['Location'].apply(lambda address: self.standardize_address(address))
        cleaned_nhadat247_thue['Standardized_Location'] = cleaned_nhadat247_thue.apply(self.fill_empty_location, axis=1)
        # cleaned_nhadat247_thue.to_csv('./processed_data/nhadat247/processed_nhadat247_thue.csv', index=False)
        cleaned_nhadat247_thue['Best_Match_Location'] = cleaned_nhadat247_thue.apply(self.find_best_match, axis=1)

        cleaned_nhadat247_thue[['Ward', 'District', 'City']] = cleaned_nhadat247_thue['Best_Match_Location'].apply(lambda x: pd.Series(self.split_location(x)))
        num_none = cleaned_nhadat247_thue[['Ward', 'District', 'City']].isna().sum()

        print("Number of None values in each column:")
        print(num_none)
        cleaned_nhadat247_thue.to_csv(self.output_path, index=False)
        return cleaned_nhadat247_thue
