...
class CafelandBanProcessor:
    ...
    def preprocess(self):
        cleaned_cafeland_ban = self.clean_dataframe(self.data)
        cleaned_cafeland_ban['Acreage'] = cleaned_cafeland_ban['Acreage'].str.replace('[^\d]', '', regex=True)
        cleaned_cafeland_ban = cleaned_cafeland_ban.astype(str)
        cleaned_cafeland_ban['Num_floors'] = cleaned_cafeland_ban['Num_floors'].str.replace('[^\d]', '', regex=True)
        cleaned_cafeland_ban['Num_rooms'] = cleaned_cafeland_ban['Num_rooms'].str.replace('[^\d]', '', regex=True)
        cleaned_cafeland_ban['Num_toilets'] = cleaned_cafeland_ban['Num_toilets'].str.replace('[^\d]', '', regex=True)
        cleaned_cafeland_ban['Street_size'] = cleaned_cafeland_ban['Street_size'].str.replace('[^\d]', '', regex=True)

        cleaned_cafeland_ban['Created_date'] = cleaned_cafeland_ban['Created_date'].str.replace('Cập nhật: ', '')
        cleaned_cafeland_ban['Created_date'] = pd.to_datetime(cleaned_cafeland_ban['Created_date'], format='%d-%m-%Y')
        cleaned_cafeland_ban['Created_date'] = cleaned_cafeland_ban['Created_date'].dt.strftime('%Y-%m-%d')
        cleaned_cafeland_ban =  cleaned_cafeland_ban.sort_values(by='Created_date', ascending=False)
        # print(cleaned_cafeland_ban['Price'].unique())
        cleaned_cafeland_ban['Price'] = cleaned_cafeland_ban.apply(self.convert_to_vnd,axis=1)
        cleaned_cafeland_ban['Location'] = cleaned_cafeland_ban['Location'].apply(self.clean_location)
        # cleaned_cafeland_ban['Location'].to_csv('./processed_data/cafeland/processed_cafeland_ban.csv', index=False)
        cleaned_cafeland_ban['Standardized_Location'] = cleaned_cafeland_ban['Location'].apply(lambda address: self.standardize_address(address))
        cleaned_cafeland_ban['Standardized_Location'] = cleaned_cafeland_ban.apply(self.fill_empty_location, axis=1)
        # cleaned_cafeland_ban.to_csv('./processed_data/cafeland/processed_cafeland_ban.csv', index=False)
        cleaned_cafeland_ban['Best_Match_Location'] = cleaned_cafeland_ban.apply(self.find_best_match, axis=1)

        cleaned_cafeland_ban[['Ward', 'District', 'City']] = cleaned_cafeland_ban['Best_Match_Location'].apply(lambda x: pd.Series(self.split_location(x)))
        num_none = cleaned_cafeland_ban[['Ward', 'District', 'City']].isna().sum()

        print("Number of None values in each column:")
        print(num_none)
        cleaned_cafeland_ban.to_csv(self.output_path, index=False)
        return cleaned_cafeland_ban
