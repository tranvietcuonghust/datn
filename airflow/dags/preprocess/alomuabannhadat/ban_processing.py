...
class AlomuabannhadatBanProcessor:
    def __init__(self, input_path, output_path):
        self.name = "alomuabannhadat_ban_daily"
    ...
    def clean_location(self,input_string):
        ...
        # cleaned_string = cleaned_string.replace('Thành phố', '')
    ...
    def preprocess(self):
        cleaned_alomuabannhadat_ban = self.clean_dataframe(self.data)
        cleaned_alomuabannhadat_ban['Acreage'] = cleaned_alomuabannhadat_ban['Acreage'].str.replace('[^\d]', '', regex=True)
        cleaned_alomuabannhadat_ban = cleaned_alomuabannhadat_ban.astype(str)
        cleaned_alomuabannhadat_ban['Horizontal_length'] = cleaned_alomuabannhadat_ban['Horizontal_length'].str.replace('[^\d]', '', regex=True)
        cleaned_alomuabannhadat_ban['Vertical_length'] = cleaned_alomuabannhadat_ban['Vertical_length'].str.replace('[^\d]', '', regex=True)
        cleaned_alomuabannhadat_ban['Num_floors'] = cleaned_alomuabannhadat_ban['Num_floors'].str.replace('[^\d]', '', regex=True)
        cleaned_alomuabannhadat_ban['Num_rooms'] = cleaned_alomuabannhadat_ban['Num_rooms'].str.replace('[^\d]', '', regex=True)
        cleaned_alomuabannhadat_ban['Num_toilets'] = cleaned_alomuabannhadat_ban['Num_toilets'].str.replace('[^\d]', '', regex=True)

        cleaned_alomuabannhadat_ban['Created_date'] = cleaned_alomuabannhadat_ban['Created_date'].str.replace('/', '-')
        cleaned_alomuabannhadat_ban['Created_date'] = pd.to_datetime(cleaned_alomuabannhadat_ban['Created_date'], format='%d-%m-%Y')
        cleaned_alomuabannhadat_ban['Created_date'] = cleaned_alomuabannhadat_ban['Created_date'].dt.strftime('%d-%m-%Y')
        cleaned_alomuabannhadat_ban =  cleaned_alomuabannhadat_ban.sort_values(by='Created_date', ascending=False)
        # print(cleaned_alomuabannhadat_ban['Price'].unique())
        cleaned_alomuabannhadat_ban['Price'] = cleaned_alomuabannhadat_ban.apply(self.convert_to_vnd,axis=1)
        cleaned_alomuabannhadat_ban['Location'] = cleaned_alomuabannhadat_ban['Location'].apply(self.clean_location)
        # cleaned_alomuabannhadat_ban['Location'].to_csv('./processed_data/alomuabannhadat/processed_alomuabannhadat_ban.csv', index=False)
        cleaned_alomuabannhadat_ban['Standardized_Location'] = cleaned_alomuabannhadat_ban['Location'].apply(lambda address: self.standardize_address(address))
        cleaned_alomuabannhadat_ban['Standardized_Location'] = cleaned_alomuabannhadat_ban.apply(self.fill_empty_location, axis=1)
        # cleaned_alomuabannhadat_ban.to_csv('./processed_data/alomuabannhadat/processed_alomuabannhadat_ban.csv', index=False)
        cleaned_alomuabannhadat_ban['Best_Match_Location'] = cleaned_alomuabannhadat_ban.apply(self.find_best_match, axis=1)

        cleaned_alomuabannhadat_ban[['Ward', 'District', 'City']] = cleaned_alomuabannhadat_ban['Best_Match_Location'].apply(lambda x: pd.Series(self.split_location(x)))
        num_none = cleaned_alomuabannhadat_ban[['Ward', 'District', 'City']].isna().sum()

        print("Number of None values in each column:")
        print(num_none)
        cleaned_alomuabannhadat_ban.to_csv(self.output_path, index=False)
        return cleaned_alomuabannhadat_ban   
