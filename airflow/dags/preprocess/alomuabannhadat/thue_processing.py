...
class AlomuabannhadatThueProcessor:
    ...
    def preprocess(self):
        cleaned_alomuabannhadat_thue = self.clean_dataframe(self.data)
        cleaned_alomuabannhadat_thue['Acreage'] = cleaned_alomuabannhadat_thue['Acreage'].str.replace('[^\d]', '', regex=True)
        cleaned_alomuabannhadat_thue = cleaned_alomuabannhadat_thue.astype(str)
        cleaned_alomuabannhadat_thue['Horizontal_length'] = cleaned_alomuabannhadat_thue['Horizontal_length'].str.replace('[^\d]', '', regex=True)
        cleaned_alomuabannhadat_thue['Vertical_length'] = cleaned_alomuabannhadat_thue['Vertical_length'].str.replace('[^\d]', '', regex=True)
        cleaned_alomuabannhadat_thue['Num_floors'] = cleaned_alomuabannhadat_thue['Num_floors'].str.replace('[^\d]', '', regex=True)
        cleaned_alomuabannhadat_thue['Num_rooms'] = cleaned_alomuabannhadat_thue['Num_rooms'].str.replace('[^\d]', '', regex=True)
        cleaned_alomuabannhadat_thue['Num_toilets'] = cleaned_alomuabannhadat_thue['Num_toilets'].str.replace('[^\d]', '', regex=True)

        cleaned_alomuabannhadat_thue['Created_date'] = cleaned_alomuabannhadat_thue['Created_date'].str.replace('/', '-')
        cleaned_alomuabannhadat_thue['Created_date'] = pd.to_datetime(cleaned_alomuabannhadat_thue['Created_date'], format='%d-%m-%Y')
        cleaned_alomuabannhadat_thue['Created_date'] = cleaned_alomuabannhadat_thue['Created_date'].dt.strftime('%d-%m-%Y')
        cleaned_alomuabannhadat_thue =  cleaned_alomuabannhadat_thue.sort_values(by='Created_date', ascending=False)
        # print(cleaned_alomuabannhadat_thue['Price'].unique())
        cleaned_alomuabannhadat_thue['Price'] = cleaned_alomuabannhadat_thue.apply(self.convert_to_vnd,axis=1)
        cleaned_alomuabannhadat_thue['Location'] = cleaned_alomuabannhadat_thue['Location'].apply(self.clean_location)
        # cleaned_alomuabannhadat_thue['Location'].to_csv('./processed_data/alomuabannhadat/processed_alomuabannhadat_thue.csv', index=False)
        cleaned_alomuabannhadat_thue['Standardized_Location'] = cleaned_alomuabannhadat_thue['Location'].apply(lambda address: self.standardize_address(address))
        cleaned_alomuabannhadat_thue['Standardized_Location'] = cleaned_alomuabannhadat_thue.apply(self.fill_empty_location, axis=1)
        # cleaned_alomuabannhadat_thue.to_csv('./processed_data/alomuabannhadat/processed_alomuabannhadat_thue.csv', index=False)
        cleaned_alomuabannhadat_thue['Best_Match_Location'] = cleaned_alomuabannhadat_thue.apply(self.find_best_match, axis=1)

        cleaned_alomuabannhadat_thue[['Ward', 'District', 'City']] = cleaned_alomuabannhadat_thue['Best_Match_Location'].apply(lambda x: pd.Series(self.split_location(x)))
        num_none = cleaned_alomuabannhadat_thue[['Ward', 'District', 'City']].isna().sum()

        print("Number of None values in each column:")
        print(num_none)
        cleaned_alomuabannhadat_thue.to_csv(self.output_path, index=False)
        return cleaned_alomuabannhadat_thue   
