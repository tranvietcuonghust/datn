...
from crawler_daily.spiders.alomuabannhadat_ban import CrawlerSpider as alomuabannhadat_ban_spider
from crawler_daily.spiders.alomuabannhadat_thue import CrawlerSpider as alomuabannhadat_thue_spider
from crawler_daily.spiders.alomuabannhadat_dat import CrawlerSpider as alomuabannhadat_dat_spider
from crawler_daily.spiders.cafeland_ban import CrawlerSpider as cafeland_ban_spider
from crawler_daily.spiders.cafeland_thue import CrawlerSpider as cafeland_thue_spider
from crawler_daily.spiders.nhadat247_ban import CrawlerSpider as nhadat247_ban_spider
from crawler_daily.spiders.nhadat247_thue import CrawlerSpider as nhadat247_thue_spider
from crawler_daily.spiders.nhadatban24h_ban import CrawlerSpider as nhadatban24h_ban_spider
from crawler_daily.spiders.nhadatban24h_thue import CrawlerSpider as nhadatban24h_thue_spider
from crawler_daily.spiders.nhadatvui_ban import CrawlerSpider as nhadatvui_ban_spider
from crawler_daily.spiders.nhadatvui_thue import CrawlerSpider as nhadatvui_thue_spider
...
def load_data_to_bigquery(date):
    ...
    load_csv_to_bigquery('bds', 'bds_3','temp_bds', f"/opt/airflow/data/deduplicated/deduplicated_{date}/deduplicated_dataframe_{date}.csv", 'bds-warehouse-424208')



def upload_to_minio(file_path, date, spider_name):
    minio_url = 'minio:9000'  # Replace with your MinIO server URL
    access_key = 'wcNt85aVYrEn57uMT238'  # Replace with your MinIO access key
    ...
    try:
        ...
        # Upload the file
        object_name = f'{spider_name}_{date}.csv'
...
def run_scrapy(date,spider):
    ...
    reactor.run()
    upload_to_minio(output_path, date, spider.name)
...
for spider in ban_spiders:
    run_spider_task = PythonOperator(
        ...
        python_callable=run_scrapy,
        op_kwargs={'date':'{{ ds }}','spider': spider},
        dag=dag,
    )
...
for processor in processors:
    process_data_task = PythonOperator(
        ...
        op_kwargs={'date': '{{ ds }}', 'processor': processor}, 
        dag=dag
    )
...
#     python_callable=run_scrapy,
#     op_kwargs={'date':'{{ ds }}','ban_spiders': ban_spiders,'thue_spiders': thue_spiders},
#     dag=dag,
# )
