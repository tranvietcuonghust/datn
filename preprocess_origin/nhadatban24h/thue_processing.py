...
def clean_dataframe(df):
    ...
    for column in df_cleaned.columns:
        if df_cleaned[column].dtype == 'object':  # Only apply to object columns, which are typically strings
            ...
            df_cleaned[column] = re.sub(r' {2,}', ' ', sample_string)
...
def standardize_address(address):
  ...
  address = re.sub('"', "", address)
  address = re.sub('HCM', "Hồ Chí Minh", address,flags=re.IGNORECASE)
...
#   address = re.sub(district_pattern, '', address)
  ...
  return result
nhadatban24h_thue = pd.read_csv('./crawled_data/nhadatban24h_thue.csv')
# nhadatban24h_thue.head(2).to_csv('./crawled_data/first_two_rows.csv', index=False)
cleaned_nhadatban24h_thue = clean_dataframe(nhadatban24h_thue)
cleaned_nhadatban24h_thue = cleaned_nhadatban24h_thue.drop(['Facade'], axis=1)
cleaned_nhadatban24h_thue['Acreage'] = cleaned_nhadatban24h_thue['Acreage'].str.replace('[^\d]', '', regex=True)
cleaned_nhadatban24h_thue = cleaned_nhadatban24h_thue.astype(str)
cleaned_nhadatban24h_thue['Horizontal_length'] = cleaned_nhadatban24h_thue['Horizontal_length'].str.replace('[^\d]', '', regex=True)
cleaned_nhadatban24h_thue['Vertical_length'] = cleaned_nhadatban24h_thue['Vertical_length'].str.replace('[^\d]', '', regex=True)
cleaned_nhadatban24h_thue['Num_floors'] = cleaned_nhadatban24h_thue['Num_floors'].str.replace('[^\d]', '', regex=True)
cleaned_nhadatban24h_thue['Num_rooms'] = cleaned_nhadatban24h_thue['Num_rooms'].str.replace('[^\d]', '', regex=True)
cleaned_nhadatban24h_thue['Num_toilets'] = cleaned_nhadatban24h_thue['Num_toilets'].str.replace('[^\d]', '', regex=True)
cleaned_nhadatban24h_thue['Street_size'] = cleaned_nhadatban24h_thue['Street_size'].str.replace('[^\d]', '', regex=True)
cleaned_nhadatban24h_thue['Created_date'] = cleaned_nhadatban24h_thue['Created_date'].str.replace('/', '-')
cleaned_nhadatban24h_thue['Created_date'] = pd.to_datetime(cleaned_nhadatban24h_thue['Created_date'], format='%d-%m-%Y')
cleaned_nhadatban24h_thue['Created_date'] = cleaned_nhadatban24h_thue['Created_date'].dt.strftime('%Y-%m-%d')
cleaned_nhadatban24h_thue= cleaned_nhadatban24h_thue.sort_values(by='Created_date', ascending=False)
# print(cleaned_nhadatban24h_thue['Price'].unique())
cleaned_nhadatban24h_thue['Price'] = cleaned_nhadatban24h_thue.apply(convert_to_vnd,axis=1)

cleaned_nhadatban24h_thue['Standardized_Location'] = cleaned_nhadatban24h_thue['Location'].apply(lambda address: standardize_address(address))
cleaned_nhadatban24h_thue.to_csv('./processed_data/nhadatban24h/processed_nhadatban24h_thue.csv', index=False)
cleaned_nhadatban24h_thue = pd.read_csv('./processed_data/nhadatban24h/processed_nhadatban24h_thue.csv')
cleaned_nhadatban24h_thue['Standardized_Location'] = cleaned_nhadatban24h_thue.apply(fill_empty_location, axis=1)
cleaned_nhadatban24h_thue['Best_Match_Location'] = cleaned_nhadatban24h_thue.apply(find_best_match, axis=1)
cleaned_nhadatban24h_thue[['Ward', 'District', 'City']] = cleaned_nhadatban24h_thue['Best_Match_Location'].apply(lambda x: pd.Series(split_location(x)))
num_none = cleaned_nhadatban24h_thue[['Ward', 'District', 'City']].isna().sum()
