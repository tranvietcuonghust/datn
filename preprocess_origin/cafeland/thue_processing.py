...
cafeland_thue = pd.read_csv('./crawled_data/cafeland_thue.csv',encoding='utf-8')

cleaned_cafeland_thue = clean_dataframe(cafeland_thue)
...
cleaned_cafeland_thue['Price'] = cleaned_cafeland_thue.apply(convert_to_vnd,axis=1)
cleaned_cafeland_thue['Location'] = cleaned_cafeland_thue['Location'].apply(clean_location)
# cleaned_cafeland_thue['Location'].to_csv('./processed_data/cafeland/processed_cafeland_thue.csv', index=False)
cleaned_cafeland_thue['Standardized_Location'] = cleaned_cafeland_thue['Location'].apply(lambda address: standardize_address(address))
cleaned_cafeland_thue['Standardized_Location'] = cleaned_cafeland_thue.apply(fill_empty_location, axis=1)
cleaned_cafeland_thue.to_csv('./processed_data/cafeland/processed_cafeland_thue.csv', index=False)
cleaned_cafeland_thue['Best_Match_Location'] = cleaned_cafeland_thue.apply(find_best_match, axis=1)

cleaned_cafeland_thue[['Ward', 'District', 'City']] = cleaned_cafeland_thue['Best_Match_Location'].apply(lambda x: pd.Series(split_location(x)))
